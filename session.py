...
MB_VERSIONS = ('GT-N7000', 'SM-N9000', 'GT-I9220', 'GT-I9100')
IMGR_ESOLUTIONS = ('720x1280', '320x480', '480x800', '1024x768', '1280x720', '768x1024', '480x320')
DPIS = ('120', '160', '320', '240')


def generate_user_agent():
    ver = random.choice(MB_VERSIONS)
    res = random.choice(IMGR_ESOLUTIONS)
    dpi = random.choice(DPIS)

    insta_version = '.'.join([str(p) for p in ['4', randint(1, 2), randint(0, 2)]])
    ...
    return 'Instagram %s Android (%s; %s; %s; samsung; %s; %s; smdkc210; en_US)' % \
           (insta_version, and_version, dpi, res, ver, ver)
...
class PyinstagramSession(object):
